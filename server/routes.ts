...
export async function registerRoutes(
  ...
  app.get("/api/admin/export-data", async (req, res) => {
    try {
      ...
      const exportTables = [
        "employees", "projects", "project_monthly", "pipeline_opportunities",
        "timesheets", "costs", "milestones", "cx_ratings", "resource_costs",
        "scenarios", "scenario_adjustments", "reference_data", "data_sources",
        "kpis", "forecasts", "resource_plans", "rate_cards",
      ];
      ...
      exportData._meta = [{ exportedAt: new Date().toISOString(), totalRecords, tables: exportTables.length }];

      res.setHeader("Content-Type", "application/json");
      res.setHeader("Content-Disposition", `attachment; filename=financehub-export-${new Date().toISOString().slice(0, 10)}.json`);
      res.json(exportData);
    } catch (err: any) {
  ...
  app.post("/api/admin/import-data", importUpload.single("file"), async (req, res) => {
    try {
      if (!req.session?.userId) return res.status(401).json({ message: "Not authenticated" });
      if (req.session.role !== "admin") return res.status(403).json({ message: "Admin access required" });
      ...
      const importOrder = [
        "employees", "projects", "project_monthly", "pipeline_opportunities",
        "reference_data", "data_sources", "kpis", "forecasts", "resource_plans", "rate_cards",
        "timesheets", "costs", "milestones", "cx_ratings", "resource_costs",
      ...
      if (!importData._meta || !Array.isArray(importData._meta)) {
        return res.status(400).json({ message: "Invalid export file — missing _meta. Use a file exported from FinanceHub." });
      }

      await db.transaction(async (trx) => {
        const clearOrder = [...importOrder].reverse();
        clearOrder.push("messages", "conversations", "onboarding_steps");
        for (const table of clearOrder) {
          try { await trx(table).del(); } catch {}
        }
        ...
        for (const table of importOrder) {
          if (counts[table] && counts[table] > 0) {
            try {
              await trx.raw(`SELECT setval(pg_get_serial_sequence('${table}', 'id'), COALESCE((SELECT MAX(id) FROM "${table}"), 1))`);
            } catch {}
          }
        }
      });

      const totalImported = Object.values(counts).reduce((sum, c) => sum + c, 0);
      res.json({
        message: `Imported ${totalImported} records across ${Object.keys(counts).filter(k => counts[k] > 0).length} tables`,
        counts,
      });
    } catch (err: any) {
