...
- **Financial Logic:** Incorporates Australian FY specific calculations (Jul-Jun), pipeline classifications with win probabilities, VAT categories, and distinct billing types (Fixed, T&M, LH). Gross Margin (GM) is a key metric. YTD revenue/cost calculations use raw timesheet data (no day-level pro-rating). Dashboard RAG warning threshold is 90% of target (amber zone). Finance page GP% RAG: Green ≥30%, Amber ≥15%, Red <15%.
- **What-If Scenarios with YTD Projections:** Combines YTD actual revenue with remaining contracted revenue and weighted pipeline forecast for projected financial position (Projected Total = YTD + Remaining Contracted + Pipeline Weighted). GP fallback for pipeline opportunities without GP data is a fixed 20% margin (flagged in UI). Features summary cards and a monthly projection table.
- **Monthly Trend Projections:** Dashboard monthly trend chart shows actual data for elapsed months and projected values (dashed lines) for future months based on historical averages or existing project_monthly data.
...
- **VAT Target Tracking & Overview:** Manages per-VAT quarterly financial targets (GM Contribution, Revenue, GM%) with OK/Good/Great/Amazing tiers. VAT Overview page displays cumulative YTD vs prorated target charts. Q4 and future quarter forecasts combine committed work from `project_monthly` with job plan projections from `resource_plans` (using `weekly_allocations`, `discounted_hourly_rate`, `hourly_gross_cost`). Week-to-month ownership uses Monday date key — only Mondays falling within the target calendar month are counted.
...
- **Partner View & Certifications:** Displays pipeline opportunities involving partners (via `partner` field or `category` containing "partner") and certified staff. Certification matching uses keyword mapping.
...
- **Live Data Import Pipeline:** Multi-step import flow accessible from Data Sources page:
  ...
  - `POST /api/import/project-status` — uploads Project Status Excel, bulk updates/creates projects. Header-driven column mapping supports columns: VPN (project code), A/D, VAT, Client, Project, Client Manager, Engagement Manager, Start date, End date, Billing Category, Work Type, Panel, Recurring, Work Order Amount. Budget/Actual/Balance columns are skipped (derived from timesheets via derive endpoint). Matches existing projects by project code or name. Handles Excel serial date numbers natively. Falls back to deterministic unique codes (IMP00001, IMP00002...) for new projects without codes.
