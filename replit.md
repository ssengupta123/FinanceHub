...
- **VAT Sales Committee Reports:** Web-based editable interface replicating the PowerPoint VAT SC Report template. Supports 7 VATs (DAFF, SAU, VICGov, DISR, Growth, P&P, Emerging) with tabbed sections for Status Overview, Risks/Issues table, Planner Tasks, and AI Suggestions. All changes are logged with user attribution. AI suggestions pull from existing pipeline_opportunities data to provide strategic advice. Database tables: `vat_reports`, `vat_risks`, `vat_action_items`, `vat_planner_tasks`, `vat_change_logs`.
